...
export default function DebitCard() {
  ...
  setShopifyOrderCreated(true); // ✅ Burada hemen lock atalım
  ...
  try {
    ...
    if (json.success) {
      navigation.replace('OrderSuccessful', { orderId: json.shopifyOrderId, selectedAddress: shippingAddress, paymentMethod: 'iyzico', });
    } else {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.header}>
        <Text style={styles.headerText}>Kredi Kartı ile Ödeme</Text>
  ...
  onNavigationStateChange={({ url }) => {
    ...
    if (url.includes('/pages/payment-success') && !shopifyOrderCreated) {
      createShopifyOrder(); // ✅ sadece ilk tetikleme çalışır
    }

    if (url.includes('/pages/payment-failed')) {
      navigation.replace('OrderNotSuccessful');
    }
  }}
  style={{ flex: 1 }}
